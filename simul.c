...
void secular (double pdf_in[], double pdf_out[]) {
  ...
  */
    ...
    rtol = 4 * DBL_EPSILON;
    while (1) {
	y = brentq (f, ll + epsilon, uu - epsilon, xtol, rtol, 1000, &stats,
		    (void *) (&args));
	...
	p = pdf_in[i + 1];
	pdf_out[i] = a;
	pdf_out[i + 1] = p / (_collapse (1 + b * a) + y * a);
    }
}
...
void MLE_t_value (double pdf_in[], double ref, double s, double pdf_out[]) {
  ...
  */
	...
	for (j = 0; j < N; j++) {
	    ...
	    p = pdf_in[2 * j + 1];
	    pdf1[2 * j] =
		a - ref - s * sigma * (1 + pow ((mu - a) / sigma, 2)) / 2;
	    pdf1[2 * j + 1] = p;
	}
	secular (pdf1, pdf2);
	for (j = 0; j < N; j++) {
	    pdf_out[2 * j + 1] = pdf2[2 * j + 1];
	}
	m = 0;
	for (j = 0; j < N; j++) {
...
double LLR_normalized_alt (double nt0, double nt1, int results_in[]) {
  ...
  */
    ...
    nt = (mu - 0.5) / sigma_pg;
    return (games / 2.0) * log ((1 + (nt - nt0) * (nt - nt0)) /
				(1 + (nt - nt1) * (nt - nt1)));
}
...
double elo_to_belo (double elo, double draw_elo, double advantage) {
    ...
    stats_t stats = { 0, 0, 0 };
    double belo =
	brentq (g, -1000, 1000, epsilon, epsilon, 1000, &stats, &qs);
...
double nelo_to_belo (double nelo, double draw_elo, double advantage) {
    ...
    stats_t stats = { 0, 0, 0 };
    double belo =
	brentq (h, -1000, 1000, epsilon, epsilon, 1000, &stats, &qs);
    assert (stats.error_num == 0);
    return belo;
}

void
be_data (double draw_ratio, double bias, double *draw_elo,
	 double *advantage) {
    double P[3];
...
void
simulate (uint64_t * prng, double alpha, double beta, double elo0,
	  double elo1, int elo_model, double pdf[], int batch, int overshoot,
...
void *sim_function (void *args) {
	...
	simulate (&prng, sim_->alpha, sim_->beta, sim_->elo0, sim_->elo1,
		  sim_->elo_model, sim_->pdf, sim_->batch, sim_->overshoot,
...
void usage () {
	...
	("simul [-h] [--alpha ALPHA] [--beta BETA] [--elo0 ELO0] [--elo1 ELO1] "
	 ...
	 "[--elo_model ELO_MODEL] " "[--seed SEED]\n");
}

void disp_elo_models (double pdf[], double pdf0[], double pdf1[], double belo, double belo0, double belo1) {	/* Assumes pentanomial */
    ...
    printf ("===         ========          ==========            =====\n");
    printf
	("Elo0      %10.5f          %10.5f       %10.5f                   \n",
	 elo0, nelo0, belo0);
    printf
	("Elo1      %10.5f          %10.5f       %10.5f                   \n",
	 elo1, nelo1, belo1);
    printf
	("Elo       %10.5f          %10.5f       %10.5f                   \n",
	 elo, nelo, belo);
}

int main (int argc, char **argv) {
    double alpha = 0.05, beta = 0.05, elo0 = 0.0, elo1 = 5.0, elo =
	0.0, draw_ratio = 0.61, bias = 0.0;
    int batch = 1;
    double p, ci;
	...
	|| !(overshoot == 0 || overshoot == 1) || batch <= 0) {
	usage ();
	return 0;
    }
    ...
    printf ("alpha      = %8.4f\nbeta       = %8.4f\nelo0       = %8.4f\n"
	    ...
	    "elo_model  =   %s\n"
	    "seed       =   %" PRIu64 "\n\n",
	    alpha, beta, elo0, elo1, elo, draw_ratio, bias, overshoot,
	    num_threads, truncate, batch,
